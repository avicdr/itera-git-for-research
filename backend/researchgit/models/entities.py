...
class PdfAnnotation(UUIDMixin, TimestampMixin, Base):
    __tablename__="pdf_annotations"; __table_args__=(Index("ix_pdf_annotation_artifact_page", "artifact_id", "page_number"),)
    workspace_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("workspaces.id",ondelete="CASCADE")); artifact_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("artifacts.id",ondelete="CASCADE")); artifact_version_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("artifact_versions.id")); page_number: Mapped[int]=mapped_column(Integer); selected_text: Mapped[str]=mapped_column(Text,default=""); anchor_data: Mapped[dict]=mapped_column(JSON,default=dict); note: Mapped[str|None]=mapped_column(Text,nullable=True); created_by: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id")); resolved_at: Mapped[datetime|None]=mapped_column(DateTime(timezone=True),nullable=True)
class DiscussionThread(UUIDMixin, TimestampMixin, Base):
    __tablename__="discussion_threads"; __table_args__=(Index("ix_discussion_scope", "workspace_id", "scope"),)
    workspace_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("workspaces.id",ondelete="CASCADE")); scope: Mapped[str]=mapped_column(String(30)); branch_id: Mapped[uuid.UUID|None]=mapped_column(Uuid,ForeignKey("branches.id")); artifact_id: Mapped[uuid.UUID|None]=mapped_column(Uuid,ForeignKey("artifacts.id")); title: Mapped[str|None]=mapped_column(String(300)); created_by: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id"))
class DiscussionMessage(UUIDMixin, TimestampMixin, Base):
    __tablename__="discussion_messages"; __table_args__=(Index("ix_discussion_message_thread", "thread_id", "created_at"),)
    thread_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("discussion_threads.id",ondelete="CASCADE")); user_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id")); content: Mapped[str]=mapped_column(Text); references: Mapped[dict]=mapped_column(JSON,default=dict)
class Notification(UUIDMixin, TimestampMixin, Base):
    __tablename__="notifications"; __table_args__=(Index("ix_notification_user_unread", "user_id", "read_at"),)
    user_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id",ondelete="CASCADE")); workspace_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("workspaces.id",ondelete="CASCADE")); type: Mapped[str]=mapped_column(String(60)); actor_id: Mapped[uuid.UUID|None]=mapped_column(Uuid,ForeignKey("users.id")); entity_type: Mapped[str]=mapped_column(String(60)); entity_id: Mapped[uuid.UUID|None]=mapped_column(Uuid); read_at: Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); metadata_json: Mapped[dict]=mapped_column("metadata",JSON,default=dict)
class ResearchReview(UUIDMixin, TimestampMixin, Base):
    __tablename__="research_reviews"; __table_args__=(Index("ix_review_workspace_status", "workspace_id", "status"),)
    workspace_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("workspaces.id",ondelete="CASCADE")); source_branch_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("branches.id")); target_branch_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("branches.id")); title: Mapped[str]=mapped_column(String(300)); description: Mapped[str|None]=mapped_column(Text); created_by: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id")); status: Mapped[str]=mapped_column(String(40),default="OPEN"); merged_at: Mapped[datetime|None]=mapped_column(DateTime(timezone=True))
class ResearchReviewDecision(UUIDMixin, TimestampMixin, Base):
    __tablename__="research_review_decisions"; __table_args__=(UniqueConstraint("review_id","reviewer_id"),)
    review_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("research_reviews.id",ondelete="CASCADE")); reviewer_id: Mapped[uuid.UUID]=mapped_column(Uuid,ForeignKey("users.id")); decision: Mapped[str]=mapped_column(String(40)); comment: Mapped[str|None]=mapped_column(Text)
