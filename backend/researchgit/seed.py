...
async def seed():
 ...
 async with SessionLocal() as s:
  u=User(name="Anoushka",email="anoushka@example.test");s.add(u);await s.flush();w=Workspace(name="Autonomous systems evidence",description="Demo research corpus",created_by=u.id);s.add(w);await s.flush();s.add(WorkspaceMember(workspace_id=w.id,user_id=u.id));main=Branch(workspace_id=w.id,name="main",created_by=u.id,is_protected=True);s.add(main);await s.flush()
  state={}
